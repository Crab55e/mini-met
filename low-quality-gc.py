...
guilds = open("guilds.json","r",encoding="utf-8")
        ...
        if m.content.startswith("!sc gchat"):
            ...
            if m.content == "!sc gchat regist":
                try: 
                    ...
                    guilds_tmp = open("guilds.json","w",encoding="utf-8")
                    ...
                    await m.channel.send(embed=discord.Embed(title="登録中にエラが発生しました",description="ID申請が完了していない場合は<@776726560929480707>へ申請を、内部エラーであると思われる場合は報告をお願いします"))
        ...
        if not m.author.bot:
            for a_guild_id_0 in guilds:
                if m.channel.id == guilds[a_guild_id_0]["gchat_channel"]:
                    gc_message_embed = discord.Embed(title=f"{m.content}",description=f"ID: {m.author.id} | Server: {m.guild.name} | **[link](https://discord.com/channels/{m.guild.id}/{m.channel.id}/{m.id}/)**")
                    gc_message_embed.set_author(name=f"{m.author}",icon_url=f"{m.author.avatar.url}")
